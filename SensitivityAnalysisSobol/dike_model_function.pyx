...
cpdef  float Muskingum(float C1, float C2, float C3,float Qn0_t1,float Qn0_t0,float Qn1_t0):
...
#        ema_logging.info('model initialized')
    ...
    def __call__(self, timestep=1, **kwargs):
        ...
        for s in self.planning_steps:
            ...
            data.update({'RfR Total Costs {}'.format(s): G.node['RfR_projects {}'.format(s)]['cost'.format(s)]})
            data.update({'Expected Evacuation Costs {}'.format(s): np.sum(EECosts)})

        return data


class DikeNetworkTS(object):
    def __init__(self):
        # planning steps
        self.num_planning_steps = 3
        self.num_events = 30
...
#                 EECList = []
                ...
                if s==0:
                    data['{}_Expected Annual Damage'.format(dike)] = [disc_EAD]
                    data['{}_Expected Number of Deaths'.format(dike)] = [END]
                    data['{}_Dike Investment Costs'.format(dike)] = [node['dikecosts {}'.format(s)]]
                    
                else:
                    data['{}_Expected Annual Damage'.format(dike)].append(disc_EAD)
                    data['{}_Expected Number of Deaths'.format(dike)].append(END)
                    data['{}_Dike Investment Costs'.format(dike)].append(node['dikecosts {}'.format(s)])
...
#                                               ): node['dikecosts {}'.format(s)]})
            ...
            if s==0:
                data['RfR Total Costs'] = [G.node['RfR_projects {}'.format(s)]['cost'.format(s)]]
                data['Expected Evacuation Costs'] = [np.sum(EECosts)]
            
            else:
                data['RfR Total Costs'].append(G.node['RfR_projects {}'.format(s)]['cost'.format(s)])
                #data['Expected Evacuation Costs'].append(np.sum(EECosts))
